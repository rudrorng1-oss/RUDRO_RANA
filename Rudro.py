...
def banner():
    print(f"{Fore.CYAN}\n== 🕸️ 𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑𝐃 𝐑𝐔𝐃𝐑𝐎 🕸️==={Style.RESET_ALL}")
    print(Fore.YELLOW + r"""
   ⣴⣾⣿⣿⣶⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⢸⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠈⢿⣿⣿⣿⣿⠏⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
...
def check_temp_expiry():
    ...
    active = []
    for line in lines:
        number, unban_time = line.strip().split(",") # DEVELOPER BY @Rudro_vai
...
""")
        try:
            ...
            print(f"❌ 𝗕𝗮𝗻 𝗳𝗮𝗶𝗹𝗲𝗱 {i+1} 𝗳𝗮𝗶𝗹𝗲𝗱: {e}") # DEVELOPER BY @Rudro_vai
            break
...
while True:
    ...
    if choice == "1":
        ban_permanent() # DEVELOPER BY @Rudro_vai
    elif choice == "2":
        ban_temporary()
    elif choice == "3":
        unban_permanent()
        ...
        print(f"{Fore.CYAN}👋 𝗘𝘅𝗶𝘁𝗶𝗻𝗴. 𝗦𝘁𝗮𝘆 𝘀𝗮𝗳𝗲, 𝐑𝐃 𝐑𝐔𝐃𝐑𝐎!")
        break
    else:
